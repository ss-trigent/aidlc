...
8. Fill `traceability.md` in the same commit as the code it describes — a table reconstructed after the fact is fiction. Update `knowledge/traceability/manifest.json` too: it records which test **files** prove an AC and is what check 4 parses, which the prose table does not replace
