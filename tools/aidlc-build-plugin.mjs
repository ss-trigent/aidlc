...
files.set(
  ...
  `---
name: aidlc-init
description: "Install the AI-DLC framework into the current repository: scaffold ai/ (personas, gates, templates, standards), the aidlc-check validator, the traceability manifest, and CI wiring. USE WHEN user invokes /aidlc-init, asks to set up / install / initialize AI-DLC, or another aidlc persona reports the framework is missing."
---
...
1. **Refuse-if-present check:** if \`ai/AI-DLC.md\` already exists, the framework is installed — offer an upgrade instead (diff \`$CLAUDE_PLUGIN_ROOT/framework/ai\` against \`ai/\` and walk the human through changes). Never blind-overwrite.
2. **Copy the framework:** \`cp -r "$CLAUDE_PLUGIN_ROOT/framework/ai" ./ai\` and \`mkdir -p tools && cp "$CLAUDE_PLUGIN_ROOT/framework/tools/aidlc-check.mjs" tools/\`.
...
4. **Seed traceability:** \`mkdir -p knowledge/traceability\`, copy \`$CLAUDE_PLUGIN_ROOT/framework/seed/manifest.json\` there, then run \`node tools/aidlc-check.mjs --write\` to generate the matrix view.
5. **Create artifact homes:** \`mkdir -p inception/product/requirements inception/product/inputs inception/stories/epics inception/stories/user-stories inception/design/screens inception/design/components knowledge/decisions\`. The design system itself is not seeded — tokens are grounded in the specific product, so \`/ux\` authors \`inception/design/tokens.css\` with the human on the first UI story, and \`aidlc-check --write\` then generates the \`tokens.json\` export their design tool imports.
6. **Wire CI:** show the human \`$CLAUDE_PLUGIN_ROOT/framework/seed/ci-step.yml\` and add the step to their workflow after dependency install. Explain that branch protection with this status as required is what makes the gates real — and that on private GitHub Free repos it needs Pro or a public repo.
7. **Point agents at it:** append the AI-DLC section to \`AGENTS.md\` (and \`CLAUDE.md\` if present): read \`ai/AI-DLC.md\` and \`ai/project-context.md\`, adopt a persona charter from \`ai/roles/\`, run \`node tools/aidlc-check.mjs\` before opening PRs, approvals are GitHub PR reviews.
8. **Verify:** \`node tools/aidlc-check.mjs\` must exit green (warnings about empty scope are expected on a fresh install).
9. **Hand off:** tell the human the gates in one sentence each and that the next step is \`/ba\` with their first customer need — and \`/ux\` once a story has UI. Note: this repo's skills come from the plugin itself — no per-repo skill files are needed.
